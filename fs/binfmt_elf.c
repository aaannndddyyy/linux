...
static int load_elf_binary(struct linux_binprm *bprm)
{
	...
	printk(KERN_WARNING "    loc->elf_ex.e_indent ");
	for (b = 0; b < sizeof(loc->elf_ex.e_ident); b++) {
		printk(KERN_WARNING "%d ", loc->elf_ex.e_ident[b]);
	}
	printk(KERN_WARNING "\n");
	printk(KERN_WARNING "    loc->elf_ex.e_type %x\n", loc->elf_ex.e_type);
	printk(KERN_WARNING "    loc->elf_ex.e_machine %x\n", loc->elf_ex.e_machine);
	printk(KERN_WARNING "    loc->elf_ex.e_version %x\n", loc->elf_ex.e_version);
	printk(KERN_WARNING "    loc->elf_ex.e_entry %x\n", loc->elf_ex.e_entry);
	printk(KERN_WARNING "    loc->elf_ex.e_phoff %x\n", loc->elf_ex.e_phoff);
	printk(KERN_WARNING "    loc->elf_ex.e_shoff %x\n", loc->elf_ex.e_shoff);
	printk(KERN_WARNING "    loc->elf_ex.e_flags %x\n", loc->elf_ex.e_flags);
	printk(KERN_WARNING "    loc->elf_ex.e_ehsize; %x\n", loc->elf_ex.e_ehsize);
	printk(KERN_WARNING "    loc->elf_ex.e_phentsize %x\n", loc->elf_ex.e_phentsize);
	printk(KERN_WARNING "    loc->elf_ex.e_phnum %x\n", loc->elf_ex.e_phnum);
	printk(KERN_WARNING "    loc->elf_ex.e_shentsize %x\n", loc->elf_ex.e_shentsize);
	printk(KERN_WARNING "    loc->elf_ex.e_shnum %x\n", loc->elf_ex.e_shnum);
	printk(KERN_WARNING "    loc->elf_ex.e_shstrndx %x\n", loc->elf_ex.e_shstrndx);

	retval = -ENOEXEC;
	/* First of all, some simple consistency checks */
	if (memcmp(loc->elf_ex.e_ident, ELFMAG, SELFMAG) != 0) {
		printk(KERN_WARNING "********* memcmp(loc->elf_ex.e_ident, ELFMAG, SELFMAG) != 0\n");
		goto out;
	}
